...
**/  
function download(parsed) {
  debug('Downloading files (this may take awhile)...')
  var data = parsed;
  forEachAsync(data, function(next, category, i, array) {
      ...
      forEachAsync(gallery.data, function(next2, mediaObj, k, array2) {
        ...
        if (isPhoto) {
          ...
          forEachAsync(imageURIs, function(next3, uri, l, array3) {
            downloadOne(uri, isPhoto, gallery.id + '/' + k, l + '.jpg', function(err, path) {
              if (err) {
                console.log(err);
                return err;
              }
              staticMedia.push(path);
    ...
    debug('Finished downloading static assets');
...
/**
  ...
  *
**/
function downloadOne(uri, isPhoto, dest, filename, cb) {
  ...
  mkdirp(dest, function(err) {
    ...
    if (isPhoto) {
      ...
      video.pipe(fs.createWriteStream(dest + filename + '.mp4'));
