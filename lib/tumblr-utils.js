  ...
  client.posts('williammatsuda', {type: type, tag: tag, limit: limit}, function(err, docs) {
    ...
    for (var obj in docs.posts) {
      ...
      if (type === 'video') {
        ...
        var photo = {};
        if (thiss.caption) {
          var stripHtml = thiss.caption.replace(/(<([^>]+)>)/ig,"")
          photo.caption = stripHtml;
        }
        photo.remoteImages = {
          sm: thiss.photos[0].alt_sizes[3].url,
          md: thiss.photos[0].alt_sizes[2].url,
          lg: thiss.photos[0].alt_sizes[1].url,
          xl: thiss.photos[0].alt_sizes[0].url
        }
        media.push(photo);
      }
    }
...
//  title: title of the category (e.g., "News")
...
client.parseAllTags = function(categories, cb) {
  ...
  forEachAsync(categories, function(next, c, i, arr) {
    ...
    console.log('Finished parsing tags');
    cb(null, media);
  });
}
