...
//    - array of image URLs in 3 sizes for photo, each accessible by the width of the respective image
  ...
  client.posts('williammatsuda', {type: type, tag: tag, limit: limit}, function(err, docs) {
    ...
    for (p in docs.posts) {
      if (type === 'video') {
        ...
        uris.push( [{500: sizes[1].url}, {250: sizes[3].url}] );
      }
    }
...
//  parses the tagFile, calling getURIsForTag on each tag, and
//  calls download() to save each media object to the public/media folder
function bundle() {
